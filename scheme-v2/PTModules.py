...
class PTBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask_global_returned is not None:
            mask_global_returned = mask_global_returned[:, :G]

            if mask_global_returned.shape[1] != G:
                mask_global_returned = torch.ones((B, G), dtype=torch.bool, device=x.device)

        if G % self.block_size != 0:
            pad_len_g = self.block_size - (G % self.block_size)
            # Pad Feature
            x_global_padded = F.pad(x_global, (0, 0, 0, pad_len_g))

            if mask_global_returned is not None:
                mask_global_padded = F.pad(mask_global_returned, (0, pad_len_g), value=False)
            else:
                mask_global_padded = torch.ones((B, G + pad_len_g), dtype=torch.bool, device=x.device)
                mask_global_padded[:, G:] = False
        else:
            x_global_padded = x_global
            pad_len_g = 0
            if mask_global_returned is not None:
                mask_global_padded = mask_global_returned
            else:
                mask_global_padded = torch.ones((B, G), dtype=torch.bool, device=x.device)

        # 重新计算维度
        G_padded = G + pad_len_g
        Num_Parent_Blocks = G_padded // self.block_size
        
        x_global_blocked = x_global_padded.view(B, Num_Parent_Blocks, self.block_size, -1)
        
        mask_global_blocked = mask_global_padded.view(B, Num_Parent_Blocks, self.block_size)

        # [B, Num, 64, 2D] -> [B, Num, 1, 64, 2D] -> [B, Num, 64, 64, 2D]
        x_global_expanded = x_global_blocked.unsqueeze(2).expand(-1, -1, self.block_size, -1, -1)
        
        # [B, Num, 64] -> [B, Num, 1, 64] -> [B, Num, 64, 64]
        mask_global_expanded = mask_global_blocked.unsqueeze(2).expand(-1, -1, self.block_size, -1)
        
        # [B * G_padded, 64, 2D]
        x_global_ready = x_global_expanded.reshape(B * G_padded, self.block_size, -1)
        mask_global_ready = mask_global_expanded.reshape(B * G_padded, self.block_size)

        x_global_ready = x_global_ready[:B*G, :, :]
        mask_global_ready = mask_global_ready[:B*G, :]
